...
Page({
    ...
    async fetchSchools(keyword, silent = false) {
        this.setData({ schoolSearching: true, schoolList: [] });
        try {
            const res = await (0, map_1.searchSchools)({ keyword }, silent);
            if (keyword !== this.data.form.schoolName.trim())
                return;
    ...
    async onSubmit() {
        const { account, password, confirmPassword, name, schoolName, schoolAddress, schoolLongitude, schoolLatitude, phone, email, } = this.data.form;
        if (!account.trim()) {
            wx.showToast({ title: '请输入账号', icon: 'none' });
            return;
        }
        if (!/^[a-zA-Z0-9_]{3,50}$/.test(account)) {
            wx.showToast({ title: '账号只能含字母/数字/下划线，3-50位', icon: 'none' });
            return;
        }
        ...
        try {
            await (0, auth_1.teacherRegister)({
