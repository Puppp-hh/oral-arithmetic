...
export const authService = {
  ...
  async studentLogin(account: string, password: string): Promise<LoginResult> {
    ...
    // 注意：密码字段在数据库中是哈希值，不能直接比较字符串
    // comparePassword 内部使用 bcrypt.compare，支持哈希验证
    // 这里不直接在 SQL 查询中验证密码，因为 bcrypt 哈希是单向的，必须先查询出哈希值再比较
  ...
  async studentRegister(dto: StudentRegisterDto): Promise<StudentPublic> {
    ...
    // 通过邀请码获取班级信息（内部已校验有效性）
    const classInfo = await classService.verifyInviteCode(dto.inviteCode);
    const [teacherRows] = await pool.execute<RowDataPacket[]>(
      `SELECT school_name, school_address, school_longitude, school_latitude
    ...
    const [result] = await pool.execute<ResultSetHeader>(
      `INSERT INTO student
         (account, password, name, class_id, grade_id,
          school_name, school_address, school_longitude, school_latitude,
          gender, current_level, account_status)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, 'active')`,
      [
        dto.account,
        pwdHash,
        ...
        classInfo.class_id,
        classInfo.grade_id,
        teacher.school_name ?? null,
        teacher.school_address ?? null,
        teacher.school_longitude ?? null,
        teacher.school_latitude ?? null,
        dto.gender ?? "unknown",
      ],
    );
  ...
  // ── 教师注册 ─────────────────────────────────────────────
  async teacherRegister(dto: TeacherRegisterDto): Promise<TeacherPublic> {
    const [exist] = await pool.execute<RowDataPacket[]>(
      "SELECT teacher_id FROM teacher WHERE account = ? LIMIT 1",
      [dto.account],
    );
    if (exist.length > 0) throw new Error("账号已存在");
    ...
    const [result] = await pool.execute<ResultSetHeader>(
      `INSERT INTO teacher
         (name, account, password, school_name, school_address, school_longitude, school_latitude, phone, email, account_status)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active')`,
      [
        dto.name,
        dto.account,
        pwdHash,
        dto.schoolName,
        dto.schoolAddress || null,
        dto.schoolLongitude ?? null,
        dto.schoolLatitude ?? null,
        dto.phone || null,
        dto.email || null,
      ],
    );
    ...
    const [newRows] = await pool.execute<RowDataPacket[]>(
      `SELECT teacher_id, name, account, email, phone, teaching_subjects,
              school_name, school_address, school_longitude, school_latitude,
              created_date, account_status
       FROM teacher WHERE teacher_id = ? LIMIT 1`,
      [newId],
    );
    return newRows[0] as TeacherPublic;
  },
  ...
  async refreshToken(
    ...
    const valid = await redisService.validateToken(
      payload.userId,
      payload.role,
      oldToken,
    );
